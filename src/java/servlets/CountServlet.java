...
public class CountServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getSession().setAttribute("studentcounter", studentDao.countStudents());
        ProfessorDao professorDao = new ProfessorDao(connect);
        request.getSession().setAttribute("professorcounter", professorDao.countProfessors());
        CourseDao courseDao = new CourseDao(connect);
        request.getSession().setAttribute("coursecounter", courseDao.countCourses());
        EnrollmentDao enrollmentDao = new EnrollmentDao(connect);
        request.getSession().setAttribute("enrollmentcounter", enrollmentDao.countEnrollments());
        TeachingDao teachingDao = new TeachingDao(connect);
        request.getSession().setAttribute("teachingcounter",teachingDao.countTeaching());
