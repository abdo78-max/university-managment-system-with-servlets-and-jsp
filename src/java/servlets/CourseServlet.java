...
public class CourseServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Course course = new Course(courseName, Integer.parseInt(creditHours));
        CourseDao courseDao = new CourseDao(connect);
        int resCourse = courseDao.addCourse(course);
        if (resCourse == 1) {
            request.getSession().setAttribute("message", "you successed to add this course"); 
        } else {
            request.getSession().setAttribute("message", "you failed to add this course");
        }
